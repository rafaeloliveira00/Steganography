from image import bytes_manipulation as bm
import message
import cv2 as cv


def sequence_hide(image_file, result_file, message_file):
    ...
    modified_frame = bm.hide_in_frame(frame, message_bytes)
...
def sequence_retrieve(image_file, length):
    ...
    retrieved_data = bm.retrieve_in_frame(frame, length)

    # create the file
    result = message.write_file('retrieved_data', retrieved_data)
